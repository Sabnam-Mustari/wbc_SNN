...
class Link:
	...
	def __init__(self, terminals, prevLayerN,threshold1, tau1,timeStep,dinc):
		global  v, tau
		v= threshold1
		tau = tau1
		...
		dincLimit = terminals*dinc
		self.weights = wMin + np.random.uniform(wMin, wMax, terminals)
		self.delays = np.array(range(1,dincLimit+1,dinc))
		#self.delays = np.arange(1, terminals+1, 1)
