...
class SpikeProp:
    def __init__(self, OutNeurons, ts,deltaT, tau, terminals):
        global NoOutNeurons, timeStep,timeLimit
    ...
    def errorFMSE(self, actualSpike, expSpikeT):
        ...
        return error
    @classmethod
    def setTimeLimit(self, deltaT, tau, terminals):
        global timeLimit
        timeLimit = deltaT + 2 *(tau + terminals)
    ...
    @classmethod
    def train(self, network, inputS, outputS, learningR, epochs,sample, benign, maligant):
        global learningRate
        avg =0
        correctCount =0
        ...
        for e in range(epochs):
            ...
            for inIndex in range(lenTimeSeq):
                ...
                if outputS[inIndex] == 2:
                    expSpikes[0] = benign
                else:
                    expSpikes[0] = maligant
                ...
                #print('layer', inIndex, 'Expected spikes: ', expSpikes)
                avg += predSpikes
                # recognize class
                if (outputS[inIndex] == 2) and (predSpikes <= benign):
                    correctCount += 1
                elif (outputS[inIndex] == 4) and (predSpikes >= maligant):
                    correctCount += 1
                else:
                    correctCount += 0
                # the spikes should be reset after each example
                network.resetSpikeTimeNet()
            cont = correctCount / 341.0;

            # the spikes should be reset after each example
            #network.resetSpikeTimeNet()
        #averageTime = avg / sample
        print ("averageTime:", cont)
    @classmethod
    def test(self, network, inputS, outputS, learningR, sample, benign,  maligant):
        global learningRate
        learningRate = learningR
        lenTimeSeq, inNeurons = inputS.shape
        totalError = 0
        ...
        for inIndex in range(lenTimeSeq):
            ...
            print('layer', inIndex, ' input', inLayer)
            print('layer', inIndex, 'Actual spikes: ', predSpikes, 'Expected spikes: ', expSpikes)
            #print('layer', inIndex, 'Expected spikes: ', expSpikes)
            ...
            # the spikes should be reset after each example
            network.resetSpikeTimeNet()
        cont = correctCount / 342.0;
        convergence = totalError / sample;
        print('% Correct findings ', cont)
